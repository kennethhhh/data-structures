...
class TestHeap(unittest.TestCase):
    ...
    def test_12_heap_sort_empty_peek(self):
        test_heap = MaxHeap()
        self.assertEqual(test_heap.peek(),None)

    def test_13_enqueue_full(self):
        test_heap = MaxHeap(0)
        test_heap.build_heap([2,3])
        self.assertFalse(test_heap.enqueue(9))

    def test_14_dequeue_empty(self):
        test_heap = MaxHeap()
        self.assertEqual(test_heap.dequeue(),None)

    def test_15_1item(self):
        test_heap = MaxHeap()
        test_heap.build_heap([1])
        self.assertEqual(test_heap.contents(),[1])

    def test_16_buildheap(self):
        test_heap = MaxHeap()
        test_heap.build_heap([3,2,1])
        self.assertEqual(test_heap.contents(),[3,2,1])
        alist=[4,5,6]
        test_heap.heap_sort_ascending(alist)
        self.assertEqual(alist,[4,5,6])
