...
class TestList(unittest.TestCase):
    ...
    def test_myown2(self):
        ht=HashTable(7)
        ht.insert("cat",9)
        ht.insert("elephant",3)
        self.assertEqual(ht.get_num_items(),2)
        ht.insert("cyy",4)
        #print(ht.hash_table)
        self.assertEqual(ht.get_index("cyy"),0)
        self.assertEqual(ht.get_value("cyy"),[4])
        self.assertEqual(ht.get_all_keys(),['cyy', 'cat', 'elephant'])
        self.assertEqual(ht.get_value("cat"),[9])

    def test_myown3(self):
        ht=HashTable(7)
        ht.insert("cat",1)
        ht.insert("cat",2)
        ht.insert("cat",3)
        self.assertEqual(ht.get_value("cat"),[1,2,3])
        self.assertEqual(ht.get_index("cat"), 3)

    def test_myown4(self):
        ht=HashTable(7)
        ht.insert("cat",1)
        ht.insert("cat",2)
        ht.insert("cat",3)
