...
class TestTsort(unittest.TestCase):
    ...
    def test_05(self):
        input = ['123', '101', '101', '102', '102', '101']
        with self.assertRaises(ValueError):
            self.assertEqual(tsort(input).strip,"input contains a cycle")

    def test_06(self):
        input = ['123', '101', '101', '102', '102']
        with self.assertRaises(ValueError):
            self.assertEqual(tsort(input).strip,"input contains an odd number of tokens")

    def test_07(self):
        input = []
        with self.assertRaises(ValueError):
            self.assertEqual(tsort(input).strip, "input contains no edges")
